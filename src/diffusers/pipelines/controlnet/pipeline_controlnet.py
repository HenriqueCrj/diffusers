...
def prepare_image_catvton(image):
...
class StableDiffusionControlNetPipelineModified(
    DiffusionPipeline,
    StableDiffusionMixin,
    TextualInversionLoaderMixin,
    StableDiffusionLoraLoaderMixin,
    ...
    def __call__(
        self,
        catvton_image: Union[PIL.Image.Image, torch.Tensor],
        ...
        **kwargs,
        ...
        catvton_image, condition_image, mask = self.check_inputs(catvton_image, condition_image, mask, width, height)
        catvton_image = prepare_image_catvton(catvton_image).to(self.device, dtype=self.weight_dtype)
        condition_image = prepare_image_catvton(condition_image).to(self.device, dtype=self.weight_dtype)
        mask = prepare_mask_image(mask).to(self.device, dtype=self.weight_dtype)
        # Mask catvton_image
        masked_image = catvton_image * (mask < 0.5)
        # VAE encoding
        ...
        del catvton_image, mask, condition_image
        # Concatenate latents
        masked_latent_concat = torch.cat([masked_latent, condition_latent], dim=concat_dim)
        mask_latent_concat = torch.cat([mask_latent, torch.zeros_like(mask_latent)], dim=concat_dim)

        if (self.do_classifier_free_guidance):
            #CATVTPM classifier free guidance
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # prepare the input for the inpainting model #CATVTON
                inpainting_latent_model_input = torch.cat([latent_model_input, mask_latent_concat, masked_latent_concat], dim=1)
        ...
        if not output_type == "latent":
            ...
            # Decode the final latents CATVTON
